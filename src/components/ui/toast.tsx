...
function Toast({ className, toast: toastItem, ...props }: ToastPrimitive.Root.Props) {
  return (
    <ToastPrimitive.Root
      ...
      toast={toastItem}
      className={cn(
        "group/toast pointer-events-auto absolute top-0 left-0 z-[calc(1000-var(--toast-index))] w-full origin-top rounded-lg border bg-popover text-popover-foreground shadow-lg will-change-transform outline-none select-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50",
        ...
        toastItem?.type ? TOAST_TYPE_CLASSES[toastItem.type] : undefined,
        className,
      )}
      {...props}
    />
  ...
  return (
    <span
      ...
      className={cn(
        ...
        type ? TOAST_ICON_CLASSES[type] : undefined,
      )}
    >
      {icon}
    </span>
