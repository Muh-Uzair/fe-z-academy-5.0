...
- **Student** — returns the single review left by the logged-in student for that course. Student id is taken from the `accessToken` cookie.
- **Admin** — returns all reviews for that course (paginated).
- **Instructor** — verifies the course belongs to the caller; if yes, returns all reviews for that course (paginated). If the course does not belong to the instructor, `403` is returned.
...
### Success response — Admin / Instructor
