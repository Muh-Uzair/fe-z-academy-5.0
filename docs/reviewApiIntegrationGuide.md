...
| `GET /me`               | Student only                                                  |
| `GET /course/:courseId` | Student only                                                  |
...
## API 3a — Get my reviews

`GET /api/v1/reviews/me`

Student only. Returns a paginated, sortable, searchable, filterable list of reviews left by the logged-in student — same shape and mechanics as [API 2](#api-2--list-reviews), but always scoped to the caller; `reviewBy` cannot be overridden.
...
## API 3b — Get my review for a course

`GET /api/v1/reviews/course/:courseId`

Student only. Looks up the single review (if any) left by the **logged-in student** for the given course — the student id is taken from the `accessToken` cookie, not the URL. Useful for checking whether the current student has already reviewed a course, and rendering that review inline (e.g. to pre-fill an edit form, or show "your review" on a course page).
...
| 401         | _(see auth guide `/me` 401 rows)_                   | Access-token cookie missing/invalid/expired.         |
| 403         | `You do not have permission to perform this action` | Caller is not a student.                             |
| 404         | `Review not found`                                  | The logged-in student has no review for this course. |
